...
accel_data %>%
  ...
  knitr::kable()
```
There is no any trends apparent was oberved in this table. The activities looks stable from the begining to the end.There are two outliner in this dataset, they are data from day24 and day31. It should be excluded during anylysis.
```{r}
accel_data %>%
  group_by(day_id, day) %>%
  summarize(total_activity = sum(activity_count)) %>%
